...
public class XMLMainClass {
	...
	public static void main(String[] args) {
		// /Users/mong/Documents/storage/product.xml 읽기
		try {
			// Document 생성
			File file = new File("/Users/mong/Documents" + File.separator + "storage", "product.xml");
			...
			for(int i = 0; i < nodeList.getLength(); i++) {
				Node node = nodeList.item(i);							// Node느 #text(줄바꿈을 의미)와 <product> 태그(Element를 의미) 중 하나이다.
				if(node.getNodeType() == Node.ELEMENT_NODE) {			// Node가 <product> 태그인가?
					Map<String, Object> product = new HashMap<String, Object>();
					NodeList nodeList2 = node.getChildNodes();
					for(int j = 0; j < nodeList2.getLength(); j++) {
						Node node2 = nodeList2.item(j);					// node2는 #text(줄바꿈을 의미)와 <model, <maker>, <price>
